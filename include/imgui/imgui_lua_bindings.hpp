...
const char * RunString(const char* szLua) {
  ...
  int iStatus = luaL_loadstring(Lua.lua_state, szLua);
  if(iStatus) {
    return lua_tostring(Lua.lua_state, -1);
  ...
  endStack.clear();
#endif
  iStatus = lua_pcall( Lua.lua_state, 0, 0, 0 );
...
static void PushImguiEnums(lua_State* lState, const char* tableName) {
  lua_pushstring(Lua.lua_state, tableName);
...
#undef END_ENUM
#define START_ENUM(name) \
  lua_pushstring(Lua.lua_state, #name); \
  lua_newtable(Lua.lua_state); \
  { \
    int i = 1;
#define MAKE_ENUM(c_name,lua_name) \
  lua_pushstring(Lua.lua_state, #lua_name); \
  lua_pushnumber(Lua.lua_state, c_name); \
...
void LoadImguiBindings() {
  if (!Lua.lua_state) {
    fprintf(stderr, "You didn't assign the global Lua.lua_state, either assign that or refactor LoadImguiBindings and RunString\n");
  }
  lua_newtable(Lua.lua_state);
  luaL_setfuncs(Lua.lua_state, imguilib, 0);
  PushImguiEnums(Lua.lua_state, "constant");
  lua_setglobal(Lua.lua_state, "imgui");
